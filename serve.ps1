...
if (@($catalog.PSObject.Properties.Name) -notcontains $ModelKey) {
  throw "Unknown model: $ModelKey. Available: $(@($catalog.PSObject.Properties.Name) -join ', ')"
}
...
if ($Mtp -and -not $model.geometry.hasMtp) {
  throw "$($model.name) has no MTP layers: speculative decoding cannot be enabled."
}
...
Write-Host "  MTP      : $(if ($Mtp) { 'on (--spec-type draft-mtp)' } else { 'off (--spec-type none)' })" -ForegroundColor Cyan
...
if ($Vision) { $arguments += @('--mmproj', $mmprojPath, '--image-min-tokens', 1024) }
